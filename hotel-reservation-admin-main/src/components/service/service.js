...
import EditServiceImageModal from "./editServiceImageModal";
...
class Service extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            editServiceInfoModal: false,
            editServiceImageModal: false,
    ...
    refreshList = () => {
        fetch(`${server}apps/service/list?search=${this.state.search.name}`)
            .then(res => res.json())
            .then(data => {
    ...
    toggleAddServiceModal = () => {
        this.setState({addServiceModal: !this.state.addServiceModal})
    }

    toggleEditServiceInfoModal = () => {
        this.setState({editServiceInfoModal: !this.state.editServiceInfoModal})
    }

    toggleEditServiceImageModal = () => {
        this.setState({editServiceImageModal: !this.state.editServiceImageModal})
    }
    ...
    createItem = () => {
        ...
        this.setState({ activeItem: item, addServiceModal: !this.state.addServiceModal });
    }

    editItem = (item) => {
        this.setState({ activeItem: item, editServiceInfoModal: !this.state.editServiceInfoModal });
    };

    editImage = (item) => {
        this.setState({ activeItem: item, editServiceImageModal: !this.state.editServiceImageModal });
    }
    ...
    handleSubmit = (item) => {
        ...
        axios
            .post(`${server}apps/service/create/`, itemForm)
            .then((res) => this.refreshList());
    }

    handleSubmitInfo = (item) => {
        this.toggleEditServiceInfoModal();
        let itemNew = item;
        delete itemNew.image;

        axios
            .patch(`${server}apps/service/update/${item.id}/`, itemNew)
            .then((res) => this.refreshList());
    }

    handleSubmitImage = (item) => {
        this.toggleEditServiceImageModal();
        const itemImage = new FormData();
        itemImage.append('image', item.image);

        axios
            .patch(`${server}apps/service/update/${item.id}/`, itemImage)
            .then((res) => this.refreshList());
    }

    handleDelete = (item) => {
        axios
            .delete(`${server}apps/service/destroy/${item.id}/`)
            .then((res) => this.refreshList());
    }
    ...
    render() {
        return (
            <div>
                ...
                { this.state.editServiceImageModal ? (
                    <EditServiceImageModal
                        activeItem={this.state.activeItem}
                        toggle={this.toggleEditServiceImageModal}
                ...
                { this.state.addServiceModal ? (
                    <AddServiceModal
                        activeItem={this.state.activeItem}
                        toggle={this.toggleAddServiceModal}
                        onSave={this.handleSubmit}
                    />
                ) : null }
                { this.state.editServiceInfoModal ? (
                    <EditServiceInfoModal
                        activeItem={this.state.activeItem}
                        toggle={this.toggleEditServiceInfoModal}
